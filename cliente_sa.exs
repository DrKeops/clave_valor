...
defmodule ClienteSA do
    ...
    def lee(nodo_cliente, clave) do
        send({:cliente_sa, nodo_cliente}, {:lee, clave, 0, self()})
    ...
    @doc """
       ...
       La especificación del interfaz de la función es la siguiente :
                            (se puede utilizar también para "dialyzer")
    """
    @spec escribe_generico(node(),String.t,String.t,integer,boolean) :: String.t
    def escribe_generico(nodo_cliente, clave, nuevo_valor, n_fallos, con_hash)do
        send({:cliente_sa, nodo_cliente}, {:escribe_generico, 
                        {clave, nuevo_valor, con_hash}, n_fallos, self()})
    ...
    def escribe(nodo_cliente, clave, nuevo_valor, n_fallos) do
        escribe_generico(nodo_cliente, clave, nuevo_valor, n_fallos, false)
    end
    ...
    def escribe_hash(nodo_cliente, clave, nuevo_valor, n_fallos) do
        escribe_generico(nodo_cliente, clave, nuevo_valor, n_fallos, true)
    end
    ...
    def init(nodo_servidor_gv) do
        ...
        bucle_recepcion(nodo_servidor_gv, 0)
    end


    defp bucle_recepcion(servidor_gv, num_op) do
        receive do
            {op, param, n_fallos, pid} when (op == :lee) or 
            (op == :escribe_generico) ->
                resultado = realizar_operacion(op, param, servidor_gv, num_op, 
                	n_fallos)
                send(pid, {:resultado, resultado})
                bucle_recepcion(servidor_gv, num_op + 1)
    ...
    defp realizar_operacion(op, param, servidor_gv, num_op, n_fallos) do
        ...
        case p do
            :undefined ->  # esperamos un rato si aparece primario
                ...
                realizar_operacion(op, param, servidor_gv, num_op, n_fallos)

            nodo_primario ->   # enviar operación a ejecutar a primario
                send({:servidor_sa, nodo_primario}, {op, num_op, param, 
                	Node.self()})

                # recuperar resultado
                receive do
                    {:resultado, :no_soy_primario_valido} ->
                        realizar_operacion(op, param, servidor_gv, num_op, 
                        	n_fallos)
